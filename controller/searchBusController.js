...
async function searchSchedule(req,res){
  try{
    ...
    if (sort_by) {
      ...
      const bus = await BusSchedule.findAll({
        ...
        if(time){
          ...
          filtered_bus = bus.filter(e=>e.departure_time.includes(times[0]) && e.arrival_time.includes(times[1]))
        }
        else{
          filtered_bus = bus
        }
      ...
      for(let i = 0 ; i < filtered_bus.length;i++){
          ...
          seats = (await availableSeat(departure_date,filtered_bus[i].id)).filter(e=>e==0).length
          departure_r.push({
    ...
    if(r_time){
      ...
      fil_bus = return_bus.filter(e=>e.departure_time.includes(r_times[0]) && e.arrival_time.includes(r_times[1]))
    }
    else{
      fil_bus = return_bus
    }
    for(let i = 0 ; i < fil_bus.length;i++){
      ...
      const num2 = Number(f.arrival_time.slice(0,2))
      const selisih = num2-num1
      seats = (await availableSeat(return_date,fil_bus[i].id)).filter(e=>e==0).length
      return_r.push({
          busId : f.id,
          departureTime : f.departure_time,
          arrivalTime : f.arrival_time,
