...
async function createBusSchedule(req,res){
    ...
    try{
        const {departure_city,departure_shuttle,destination_city,arrival_shuttle } = req.body
        const { id } = req.user 
        ...
        if(departureShuttle.id == arrivalShuttle.id) return res.status(400).json({status : "failed" , message :"each shuttle cannot be the same !"})
...
async function updateBusSchedule(req,res){
    const result = await sequelize.transaction({  isolationLevel: Transaction.ISOLATION_LEVELS.READ_UNCOMMITTED
        ...
        try{
            const findBus = await BusSchedule.findOne({where : {id : req.params.id}})
...
async function deleteBusSchedule (req,res){
    ...
    const result = await sequelize.transaction({  isolationLevel: Transaction.ISOLATION_LEVELS.READ_UNCOMMITTED
        ...
        try{
            ...
            const bus = await Bus.destroy({where: {
                id : findBus.bus_id
            }},{transaction :t })
...
module.exports= {createBusSchedule,showAllBus,updateBusSchedule,deleteBusSchedule,findBusSchedule}