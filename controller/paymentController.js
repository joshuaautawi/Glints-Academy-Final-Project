...
async function createPayment(req,res){
    try{
      const { order_id } = req.body
        ...
        if(!created) return res.status(400).json({status : "failed" , message : "this order has a payment before"})
...
async function showPaymentByStatus (req,res){
    try{
      ...
      if (payment.length ==0 ) return res.status(200).json({status:"success", message :`Order with status '${status}'  is not found` ,data : []})
      const result = []
    ...
    if(payment.Order.order_type == "RoundTrip"){
      ...
      result = {
        ...
        price_detail : {
          ...
          total_price : (dep_schedule.price+ret_schedule.price)*po.total_passenger,
        },
        departure_bus_name : dep_schedule.Bus.bus_name , 
        departure_bus_provider : dep_schedule.BusProvider.provide_name,
        departure_shuttle : [dep_schedule.departure_shuttle, dep_schedule.departure_city],
        departure_date : dep_schedule.departure_date,
        departure_time : [dep_schedule.departure_time,dep_schedule.arrival_time],
        return_bus_name : ret_schedule.Bus.bus_name,
        return_bus_provider :ret_schedule.BusProvider.provider_name,
        return_shuttle : [ret_schedule.departure_shuttle, ret_schedule.destination_city],
    ...
    try {
        ...
        if (transactionStatus == "capture") {
          if (fraudStatus == "challenge") {
            ...
            await Order.update({ order_status: "challenge" }, { where: { id : orderId} })
            await Payment.update({ payment_status: "challenge" }, { where: { order_id : orderId} });
          ...
          // and response with 200 OK
          await Order.update({ order_status: "expired" }, { where: { id : orderId} });
          await Payment.update({ payment_status: "expired" }, { where: { order_id : orderId} });
        } else if (transactionStatus == "pending") {
          // TODO set transaction status on your database to 'pending' / waiting payment
          // and response with 200 OK
          await Order.update({ order_status: "pending" }, { where: { id : orderId} });
          await Payment.update({ payment_status: "pending" }, { where: { order_id : orderId} });
        }
