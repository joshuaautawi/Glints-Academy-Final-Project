...
async function isLogin (req,res,next){
    ...
    try{
        ...
        if(!token)  return res.status(401).json({status : "failedd" , message : "Not authorized , please Login ",error : e})
 ...
 function authorize (role){
        ...
        return (req,res,next)=>{
            ...
            else return res.status(400).json({status : "failed", message : `this route is only authorize for ${role}`})
        }
}
